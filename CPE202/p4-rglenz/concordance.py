        ...
        for line in f:
            ...
            for word in line:
                ...
                try:
                    ...
                    if conc_table.in_table((word.rstrip()).lower()):
                        if numline not in conc_table.hash_table[conc_table.get_index((word.rstrip()).lower())][1]:
                            conc_table.hash_table[conc_table.get_index((word.rstrip()).lower())][1].append(numline) 
                    else:
    ...
    def write_concordance(self, filename):
        ...
        for i in range(len(keys)):
            ...
            if not self.check_stop(keys[i]):
                f.write(str(keys[i])+':')
                for value in self.concordance_table.get_value(keys[i]):
                    f.write(' '+str(value))
