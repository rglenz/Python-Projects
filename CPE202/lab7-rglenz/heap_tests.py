...
class TestHeap(unittest.TestCase):
    ...
    def test_12_heap_sort_ascending_01(self):
        test_heap = MaxHeap()
        list1 = [2, 9, 7, 6, 5, 8, 4, 20, 10, 56]
        test_heap.heap_sort_ascending(list1)
        self.assertEqual(list1, [2, 4, 5, 6, 7, 8, 9, 10, 20, 56])
    def test_13_build_heap_01(self):
        test_heap = MaxHeap(10)
        test_heap.build_heap([2, 9, 7, 6, 5, 8, 4, 20, 10, 56])
        insert = test_heap.enqueue(16)
        self.assertFalse(insert)
        self.assertEqual(test_heap.contents(), [56, 20, 8, 10, 9, 7, 4, 6, 2, 5])
    def test_14_enqueue_01(self):
        test_heap = MaxHeap(20)
        test_heap.build_heap([2, 9, 7, 6, 5, 8, 4, 20, 10, 56])
        insert = test_heap.enqueue(16)
        self.assertTrue(insert)
        self.assertEqual(test_heap.contents(), [56, 20, 8, 10, 16, 7, 4, 6, 2, 5,9])

    def test_17_dequeue_01(self):
        test_heap = MaxHeap()
        self.assertEqual(test_heap.peek(), None)
        self.assertEqual(test_heap.dequeue(), None)
        test_heap.build_heap([2, 9, 7, 6, 5, 8, 4, 20, 10, 56])
        self.assertEqual(test_heap.dequeue(), 56)
        self.assertEqual(test_heap.get_size(), 9)
        self.assertEqual(test_heap.contents(), [20,10,8,6,9, 7, 4, 5,2])
        self.assertEqual(test_heap.peek(), 20)
